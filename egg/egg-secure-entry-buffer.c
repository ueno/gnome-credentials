...
#include "egg-secure-entry-buffer.h"
...
/**
 * SECTION:egg-secure-entry-buffer
 * @title: EggSecureEntryBuffer
...
/**
 * EggSecureEntryBuffer:
 *
 * A #GtkEntryBuffer which uses non-pageable memory for passwords or secrets.
 */

/**
 * EggSecureEntryBufferClass:
 * @parent_class: parent class
 *
 * The class for #EggSecureEntryBuffer.
 */
...
struct _EggSecureEntryBufferPrivate
{
	gchar *text;
	gsize text_size;
	gsize text_bytes;
...
G_DEFINE_TYPE (EggSecureEntryBuffer, egg_secure_entry_buffer, GTK_TYPE_ENTRY_BUFFER);

static const gchar *
egg_secure_entry_buffer_real_get_text (GtkEntryBuffer *buffer,
                                       gsize *n_bytes)
{
	EggSecureEntryBuffer *self = EGG_SECURE_ENTRY_BUFFER (buffer);
	if (n_bytes)
...
static guint
egg_secure_entry_buffer_real_get_length (GtkEntryBuffer *buffer)
{
	EggSecureEntryBuffer *self = EGG_SECURE_ENTRY_BUFFER (buffer);
...
static guint
egg_secure_entry_buffer_real_insert_text (GtkEntryBuffer *buffer,
                                          guint position,
                                          const gchar *chars,
                                          guint n_chars)
{
	EggSecureEntryBuffer *self = EGG_SECURE_ENTRY_BUFFER (buffer);
	EggSecureEntryBufferPrivate *pv = self->pv;
	gsize n_bytes;
	gsize at;
...
static guint
egg_secure_entry_buffer_real_delete_text (GtkEntryBuffer *buffer,
                                          guint position,
                                          guint n_chars)
{
	EggSecureEntryBuffer *self = EGG_SECURE_ENTRY_BUFFER (buffer);
...
static void
egg_secure_entry_buffer_init (EggSecureEntryBuffer *self)
{
	EggSecureEntryBufferPrivate *pv;
	pv = self->pv = G_TYPE_INSTANCE_GET_PRIVATE (self, EGG_TYPE_SECURE_ENTRY_BUFFER, EggSecureEntryBufferPrivate);
...
static void
egg_secure_entry_buffer_finalize (GObject *obj)
{
	EggSecureEntryBuffer *self = EGG_SECURE_ENTRY_BUFFER (obj);
	EggSecureEntryBufferPrivate *pv = self->pv;
	...
	G_OBJECT_CLASS (egg_secure_entry_buffer_parent_class)->finalize (obj);
}

static void
egg_secure_entry_buffer_class_init (EggSecureEntryBufferClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GtkEntryBufferClass *buffer_class = GTK_ENTRY_BUFFER_CLASS (klass);

	gobject_class->finalize = egg_secure_entry_buffer_finalize;

	buffer_class->get_text = egg_secure_entry_buffer_real_get_text;
	buffer_class->get_length = egg_secure_entry_buffer_real_get_length;
	buffer_class->insert_text = egg_secure_entry_buffer_real_insert_text;
	buffer_class->delete_text = egg_secure_entry_buffer_real_delete_text;

	g_type_class_add_private (gobject_class, sizeof (EggSecureEntryBufferPrivate));
}

/**
 * egg_secure_entry_buffer_new:
 *
 * Create a new #EggSecureEntryBuffer, a #GtkEntryBuffer which uses
 * non-pageable memory for the text.
 *
 * Returns: (transfer full): the new entry buffer
 */
GtkEntryBuffer *
egg_secure_entry_buffer_new (void)
{
	return g_object_new (EGG_TYPE_SECURE_ENTRY_BUFFER, NULL);
}
